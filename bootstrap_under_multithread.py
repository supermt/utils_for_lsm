...
def start_db_bench(key_size, value_size, entry_count, bench_mark, threads, compression="none",
                   db_bench_path=DEFAULT_DB_PATH,
...
def single_run(key_size, value_size, entry_count, benchmark, thread_count, gap, db_path, db_bench_path):
    """
    ...
    try:
        ...
        db_bench_process = start_db_bench(
            key_size, value_size, entry_count, benchmark, thread_count, db_path,
            ...
            for value_size_option in value_size_options:
                for thread_count in thread_counts:
                    ...
                    result_dir = single_run(key_size_option, value_size_option,
                                            entries, benchmark, thread_count, 1.5, db_path=DEFAULT_DB_PATH,
                                            db_bench_path="./")
                    print(result_dir)
                    plot_files(result_dir, result_dir, column=2, row=2)
                    print("finish experiment with value size of " +
                          str(value_size_option) + " , total entries of " + str(entries) + ", under threads of " +
                          str(thread_count))
